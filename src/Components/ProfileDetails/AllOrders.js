...
const AllOrders = () => {
    ...
    const deleteOrders = async (email) => {
        try {
            const response = await axios.post('http://localhost:8000/user/deleteOrders', {
                email: email
            },
                {
                    headers: {
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th></th>
                        <th><button className={isDeleteAllBtnActive ? 'button disabled' : 'button'} disabled={isDeleteAllBtnActive} onClick={() => deleteOrders(user.email)}>DeleteAll</button></th>
                        {/* <th>Address: </th> */}
                    </tr>
                </thead>
                {orderDetails?.orders.map((order) => {
                    return (
                        <tbody key={order._id}>
                            <tr key={order.items[0].info.id} className='border-t border-gray-300'>
                                <td>{order.name}</td>
                                <td>{order.add1}</td>
                                <td>{order.items[0].info.name}{order.items?.length > 1 ? <Link to={`/orders/${order._id}/itemId=${order.items[0].info.id}?search=${order.email}`} className="text-sm font-bold text-[#00416a]">
                                    +{order.items.length - 1} {order.items.length - 1 > 1 ? 'items' : 'item'}
                                </Link> : ''}</td>
                                ...
                                <td><button className="button" onClick={() => navigate(`/orders/${order._id}/itemId=${order.items[0].info.id}?search=${order.email}`)}>Details...</button></td>
        ...
        <div className="cartContainer-100">
            <div>
                {orderDetails && orderDetails.orders.length > 0 ?
                    ...
                    > <h1>You dont have any orders....Order Items now</h1></div>
                }
            </div>
        </div>
    )
