...
const RestaurantCard = ({ data }) => {
  ...
  const isItemWishlisted = () => {
    if (user?.favourites?.restaurant) {
      return user.favourites.restaurant.some(
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col mt-2 flex-wrap justify-center items-center w-60 mx-auto">
        ...
        {err ? (
          <div className="p-1  rounded-md flex justify-center bg-red-600 text-white mb-4">
            {err}
          </div>
        ) : (
          ""
        ...
        <div className="mt-4 text-gray-600">
          <ul className=" text-sm flex flex-row gap-1 justify-start items-center ">
            ...
            {data.info.isOpen ? (
              <li className="bg-green-100 p-1 rounded-md hover:bg-green-200 text-gray-600">
                OPEN
              </li>
            ) : (
              <li className="bg-gray-300 p-1 rounded-md hover:bg-gray-600 hover:text-white text-gray-600">
                CLOSED
              </li>
            )}
          </ul>
