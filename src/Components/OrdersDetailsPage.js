...
const SuccessPage = () => {
  ...
  const renderItem = (item) => {
    return (
      <>
        <div
          className='flex flex-wrap justify-start gap-4 border border-text-300 mb-2 p-4 shadow-md'
          key={item.info.id}
        >
          {item.info.imageId ? (
            <Link to={`../restaurant/${item.info.restaurantData.id}`}>
              <img src={assets + item.info.imageId} className='w-32 h-28 object-fill rounded-md' alt={item.title} />
            </Link>
          ) : (
            <Link to={`../restaurant/${item.info.restaurantData.id}`}>
          ...
          <div>
            ...
            <div className='mb-0.5 text-sm'>
              Price: &#8377;{item.info.defaultPrice ? item.info.defaultPrice / 100 : item?.info?.price / 100}
            </div>
      ...
      return (
        <div key={filter._id} className='border border-text-300 mb-2 p-4 shadow-md text-sm'>
          ...
          <div className='mb-0.5'>
            <span className='font-semibold'> Order ID:</span> {filter._id}
          </div>
          <div className='mb-0.5'>
            <span className='font-semibold'> Paymode Mode:</span> {filter.paymentMode}
          </div>
          <div className='mb-0.5'>
            <span className='font-semibold mb-0.5'>Paid on: </span> {formatDateTime(filter.paymentTime)}
          </div>
          <div className='mb-0.5'>
            <span className='font-semibold mb-0.5'>Payment Status: </span> {filter.paymentStatus}
          </div>
          <div className='mb-0.5'>
            <span className='font-semibold mb-0.5'>Total Amount before Tax:</span> &#8377;
            {filter?.totalAmount?.toFixed(2)}
          </div>
          <div className='mb-0.5'>
            <span className='font-semibold mb-0.5'>Total Amount After Tax: </span>&#8377;
            {filter?.totalAmountwithCharges?.toFixed(2)}
          </div>
        </div>
      );
    });
  ...
  return (
    <div className='mx-auto p-8 bg-white rounded shadow-lg mt-2 mb-4 w-4/8'>
      <button
        className=' text-white px-6 py-1 mb-4 rounded-md transition-all duration-300 ease-in-out bg-[#00416a] hover:bg-green-700'
        onClick={() => handleGoBack()}
      >
        Back
      </button>
      <h1 className='member-name1'>order Details:</h1>
      <div className='flex flex-wrap gap-2'>
        <div className='flex-1'>
          {orderDetails && orderDetails.length > 0 ? <div>{orderDetailsFunc()}</div> : "No order details available."}
        </div>
