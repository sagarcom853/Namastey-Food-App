...
const Corousal = () => {
    ...
    useEffect(() => {
        const fetchCarouselItems = async () => {
            try {
                ...
                setItemName(response?.data?.data?.cards[0]?.card?.card?.gridElements?.infoWithStyle.info);
                localStorage.setItem('carousel-items', JSON.stringify(response?.data?.data?.cards[0]?.card?.card?.gridElements?.infoWithStyle.info));
                setLoading(false);
            } catch (error) {
                setLoading(false);
                console.log(error);
    ...
    const handleItemFilter = (url) => {
        ...
        navigate(`/collections/${collectionId}?collection_id=${collectionId}&tags=${tags}&type=rcv2`);
    };
        ...
        return (
            <Slider {...settings}>
                {itemName && itemName.map((item) => (
                    <div key={item.id} onClick={() => handleItemFilter(item.action.link)}>
                        <img src={`${corousalImageId}${item.imageId}`} alt={item.name} className='carousel-item' />
                    </div>
                ))}
            </Slider>
        );
...
const CustomPrevArrow = (props) => {
    ...
    return (
        <div className={className + " custom-prev-arrow"} onClick={handleClick}  style={arrowStyle}>
...
const CustomNextArrow = (props) => {
    ...
    return (
        <div className={className + " custom-next-arrow"} onClick={handleClick}  style={arrowStyle}>
