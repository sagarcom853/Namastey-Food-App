...
const createUser = async (req, res) => {
  try {
    const { isAdmin, name, email, password, gender, dob, add1, add2, landmark, state, country, pin, imageURL } =
      req.body;
...
const updateUser = async (req, res) => {
  try {
    const { isAdmin, name, password, gender, dob, add1, add2, landmark, pin, state, country, email, imageURL } =
      req.body;
...
const deleteByOrderId = async (req, res) => {
  ...
  try {
    ...
    const orderDetailsById = orders.filter((order) => order._id.toString() === orderId);
    if (orderDetailsById.length > 0) {
      const newOrders = orders.filter((order) => order._id.toString() !== orderId);
      existingUser.orders = newOrders;
      await existingUser.save();
      res.status(200).json({ message: "Order deleted successfully", user: existingUser });
    } else {
  ...
  try {
    ...
    if (existingUser) {
      if (type === "restaurant") {
        ...
        existingUser.favourites.items = existingUser.favourites.items.filter((item) => item.id !== itemId);
      }

      await existingUser.save();

      let message = type === "restaurant" ? "Removed from Fav Restaurants" : "Removed from favourite Items";
